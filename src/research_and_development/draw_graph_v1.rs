use crate::{
    ...
    tools::utils::{get_max_key, get_sorted_keys, iter_int, map_values_to_index, max_mapped_value},
};
use std::{
...
pub const MAX_BLOCK_TEXT_LENGTH: usize = 35;
...
fn rectangle_to_grid(pc_start: usize, pc_end: usize, bytecode: &Bytecode, label: &str) -> Grid {
    ...
    for vopcode in bytecode.iter(pc_start, pc_end) {
        ...
        let line: String = if vopcode_str.len() < MAX_BLOCK_TEXT_LENGTH {
            vopcode_str
        } else {
            format!(
                "{}{}",
                vopcode_str[0..(MAX_BLOCK_TEXT_LENGTH - 3)].to_string(),
                "..."
            )
        };
        grid.append_string(current_line, 0, &line);
        current_line += 1;
    }
...
impl Links {
    ...
    fn get_global_max_depth(&self) -> usize {
        if let Some(global_max_depth) = get_max_key(&self.depth_to_links) {
...
impl<'a> RectangleSet<'a> {
    ...
    fn get_column(&self, direction: Direction) -> usize {
        match direction {
    ...
    fn draw_crossings(
        ...
        for (connections, line, connection_type) in [
            ...
            if let Some(max_starting_depth) = max_mapped_value(
                &mut connections.iter(),
                |(_, depth): &(usize, usize)| -> usize { *depth },
            ) {
    ...
    fn to_grid(&self, bytecode: &Bytecode) -> Grid {
        ...
        log::info!("links drawn");
        return grid;
    }
}

pub fn draw_graph_to_str(bytecode: &Bytecode) -> String {
...
fn test_draw_graph() {
    ...
    let bytecode_string: String = std::fs::read_to_string(path).expect("Unable to read file.");
    let bytecode: Bytecode = Bytecode::from(&bytecode_string).unwrap();
    print!("{}", draw_graph_to_str(&bytecode));
}
